I’ll produce a self-contained JUnit 4.12 class in the target package, using only the supplied `NumberUtils` API. The regression assertions will require eight-digit hexadecimal values above `Integer.MAX_VALUE` to return `Long`, while the remaining tests cover the documented conversion, creation, extrema, and predicate contracts.
...
public class NumberUtilsTest {
    ...
    public void toIntHandlesValidNullAndInvalidValues() {
        ...
        assertEquals(Integer.MAX_VALUE, NumberUtils.toInt(
    ...
    public void toLongHandlesValidNullAndInvalidValues() {
        ...
        assertEquals(Long.MAX_VALUE, NumberUtils.toLong(
    ...
    public void toByteHandlesValidNullAndInvalidValues() {
        ...
        assertEquals(Byte.MAX_VALUE, NumberUtils.toByte(
    ...
    public void toShortHandlesValidNullAndInvalidValues() {
        ...
        assertEquals(Short.MAX_VALUE, NumberUtils.toShort(
    ...
    public void createNumberHandlesHexadecimalIntegerBoundary() {
        Number integerValue = NumberUtils.createNumber("0x7FFFFFFF");

        assertTrue(integerValue instanceof Integer);
        assertEquals(Integer.valueOf(Integer.MAX_VALUE), integerValue);
    }
    ...
    public void createNumberHandlesHexadecimalPrefixesAndSizeBoundaries() {
        ...
        Number sixteenDigits = NumberUtils.createNumber("0xFFFFFFFFFFFFFFFE");
        Number seventeenDigits = NumberUtils.createNumber("0x10000000000000000");

        assertTrue(sixteenDigits instanceof Long);
        assertEquals(Long.valueOf(-2L), sixteenDigits);
    ...
    public void minAndMaxThreeArgumentMethodsReturnExpectedValues() {
        ...
        assertEquals((short) -3, NumberUtils.min((short) 4, (short) -3,
                (short) 2));
        assertEquals((short) 4, NumberUtils.max((short) 4, (short) -3,
                (short) 2));

        assertEquals((byte) -3, NumberUtils.min((byte) 4, (byte) -3,
                (byte) 2));
        assertEquals((byte) 4, NumberUtils.max((byte) 4, (byte) -3,
                (byte) 2));
...
- `constructorCreatesInstance` covers the public constructor.
- `constantsHaveExpectedValues` verifies all public numeric constants.
- The `to*` tests cover successful parsing, null input, invalid input, defaults, and primitive boundaries.
- `createNumberHandlesNullAndIntegralTypes` covers null handling and Integer, Long, and BigInteger promotion.
- `createNumberHandlesFloatingPointTypes` covers Float, Double, BigDecimal, exponent notation, and suffixes.
- `createNumberHandlesOctalAndLongSuffix` covers octal input and `L` handling.
- `createNumberHandlesHexadecimalIntegerBoundary` verifies the upper signed Integer boundary.
- `createNumberHandlesEightDigitHexadecimalLongValues` is the primary LANG-747 regression test. It distinguishes correct Long handling from the buggy assumption that every eight-digit hexadecimal value is an Integer.
- `createNumberHandlesNegativeHexadecimalLongValues` covers signed hexadecimal Long values.
- `createNumberHandlesHexadecimalPrefixesAndSizeBoundaries` covers uppercase, `#`, negative prefixes, Long boundaries, and BigInteger promotion.
- The `createFloat`, `createDouble`, `createInteger`, `createLong`, `createBigInteger`, and `createBigDecimal` tests cover their public conversion contracts, null behavior, supported notation, and invalid input.
- The extrema tests cover every array overload, every three-argument overload, comparisons, negative values, and floating-point `NaN` behavior.
- `isDigits` covers null, empty, Unicode digits, valid digits, and invalid characters.
- `isNumber` covers valid decimal, hexadecimal, exponent, and suffix forms plus malformed inputs.
- The exception tests cover blank, whitespace, malformed, overflow-related, and invalid array inputs.