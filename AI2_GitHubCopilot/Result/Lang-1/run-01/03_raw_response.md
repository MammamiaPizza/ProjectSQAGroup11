...
- Replaced the 16-digit `0xFFFFFFFFFFFFFFFE` case with `0x7FFFFFFFFFFFFFFF`, because the former cannot be reliably represented by `Long.decode` as a positive hexadecimal value under the supplied source contract.
- Preserved the LANG-747 regression cases requiring eight-digit hexadecimal values above `Integer.MAX_VALUE` to return `Long`.
- Replaced the literal full-width Unicode digits with escaped Unicode characters so the test source is encoding-independent.
- Removed the incorrect expectation that `isNumber("1L")` is invalid. The supplied implementation and Javadoc explicitly support the `L` suffix.
- Retained the existing package, exact class name, JUnit 4 imports, public APIs, and meaningful test coverage.